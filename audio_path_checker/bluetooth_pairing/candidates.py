...
def score_candidate_with_components(
    ...
    *,
    ...
    if name == target:
        ...
        score += 80
        components.append("+80 partial target name match")
    ...
    if expected and addr and addr == expected:
        score += 10
        components.append("+10 valid DeviceAddress")
...
def determine_pairability(
    ...
    *,
    ...
    """
    Tri-state pairability for the target device population.

    UNKNOWN when Classic/AEP enumeration failed.
    """
    ...
    classic_candidates = [
        c
        for c in candidates
        if c.get("enumeration_succeeded", True)
        and (
...
def rank_candidates(
    ...
    *,
    ...
    for c in candidates:
        item = dict(c)
        item["classification"] = classify_candidate(c)
...
def select_pairable_candidate(
    ...
    *,
    ...
    """Never select CanPair=False AND IsPaired=False for normal PairAsync."""
